package com.yk.marchant.util;

// Define content split util
public class ContentUtil {
	
	public static final String IS_UTIL = " is ";

	public static final String SPACE_UTIL = " ";
	
	public static final String QUESTION_MARK_UTIL = " ?";

	public static final String HOW_MANY_UTIL = "how many";

	public static final String HOW_MUCH_UTIL = "how much";

	public static final String CREDITS_UTIL = "Credits";
	
	public static final String Wrong_Util = "I have no idea what you are talking about";
}
